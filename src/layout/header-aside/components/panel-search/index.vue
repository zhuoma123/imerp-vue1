<template>
    <!-- 主体内容 -->
    <div flex="dir:top" class="d2-layout-header-aside-content">
      <!-- 顶栏 -->
      <div
        flex-box="0"
        ...
        <el-form
          ...
          <div style="margin: -5px  0px  15px 0px; width:50%;">
            <el-input
              ref="searchInput"
              placeholder="请输入物料编号/名称/车型/品牌等等信息进行搜索"
              v-model="dataForm.name"
              class="input-with-select"
              @focus="clearCurrentRow()"
              @keyup.enter.native="searchEnterFun"
              @keyup.tab.native="searchEnterFun"
            >
      ...
      <div flex-box="1" class="d2-theme-container">
       <!-- 主体 -->
    ...
    <div flex-box="0" style="height:60px;text-align: center;">
    ...
    <el-dialog title="添加至购物车" :visible.sync="dialogFormVisible" width="35%">
      <el-form
        ...
        ref="dataForm2"
        label-width="100px"
        @keyup.enter.native="addItemToGrid()"
      >
    ...
    <el-dialog title="下单帮助文档" :visible.sync="centerDialogVisible" width="40%" center>
      <span>
        1.物料搜索框中可以输入配件的编号/名称/品牌/车型/产地/条码/图号/特征属性/描述等信息进行模糊查询.
        <span style="color:red">多个条件用空格隔开</span>. 例如 柴机油 英壳 4L。按回车
        <span style="color:red">Enter</span>键执行搜索，搜索结果在左下列表显示。
      </span>
      <br />
      <span>
        2.搜索出自己想要的结果之后，按
        <span style="color:red">Tab键</span>将光标跳转到列表页面，列表页面可以按↑↓键进行定位数据,找到需要下单的物料之后按回车Enter(或者鼠标双击)弹出确认框输入数量和单价之后按Enter进行添加.
      </span>
      <br />
      <span>3.如果左边搜索结果中没找到想要的物料,按除功能键之外的任意键光标将跳回搜索框进行再次搜索.</span>
      <br />
      <span>4.右下表格是购物车列表,在购物车列表可以直接修改数量和单价,如想删除某一行,则光标选择那行之后按Delete键进行删除.</span>
      <br />
...
export default {
  ...
  methods: {
    ...
    addItem() {
      ...
      this.mtlAddForm.amount =
        Number(1) * Number(this.mtlAddForm.price).toFixed(2);
    ...
    dataFormSubmit(isSubmit) {
      ...
      Promise.all([this.checkForm(), this.checkGrid(this.$refs.sGrid2)])
        .then(() => {
