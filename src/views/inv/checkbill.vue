	...
	<d2-container>
			...
			<el-collapse-item>
				...
				<el-form size="mini" :inline="true" :model="dataForm" ref="dataForm" @keyup.enter.native="getDataList()"
					...
					<el-form-item label="仓库" prop="warehouseId">
						<el-select v-model="dataForm.warehouseId" :loading="warehouseSel.loading" :remote-method="selWarehouse"
						           filterable
						           remote
						           placeholder="请选择">
							<el-option
		...
		<vxe-grid
			...
			<template v-slot:buttons>
				<el-button size="mini" icon="el-icon-circle-plus" v-if="$hasPermission('inv:checkbill:save')"
				           @click="addHandle">新增
				</el-button>
				<el-button type="primary" size="mini" icon="el-icon-edit" v-if="$hasPermission('inv:checkbill:save')"
				           @click="updateHandle($refs.pGrid)">修改
				</el-button>
				<el-button type="danger" size="mini" icon="el-icon-delete" v-if="$hasPermission('inv:checkbill:delete')"
				           @click="deleteHandle($refs.pGrid)">删除
				</el-button>
				<el-button type="success" size="mini" icon="el-icon-check" v-if="$hasPermission('inv:checkbill:submit')"
				           @click="submitHandle($refs.pGrid,true)">自动拣货
				</el-button>
				<el-button type="success" size="mini" icon="el-icon-check" v-if="$hasPermission('inv:checkbill:submit')"
				           @click="submitHandle($refs.pGrid,false)">人工拣货
				</el-button>
				<el-button type="info" size="mini" icon="el-icon-printer" v-if="$hasPermission('inv:checkbill:print')">打印
				</el-button>
				<el-button type="info" size="mini" icon="fa fa-file-excel-o" v-if="$hasPermission('inv:checkbill:export')"
